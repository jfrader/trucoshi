...
function* trucoTurnGeneratorSequence(truco: ITruco): Generator<ITruco, void, ITruco> {
  while (truco.answer === null) {
    // `turn` points at the next responder, so preserve that position when earlier entries disappear.
    const removedBeforeTurn = truco.players
      .slice(0, truco.turn)
      .filter((p) => p.disabled || p.abandoned).length

    truco.players = truco.players.filter((p) => !p.disabled && !p.abandoned)
    truco.setTurn(truco.turn - removedBeforeTurn)
    ...
    if (truco.turn >= truco.players.length) {
      truco.setTurn(0)
    }
