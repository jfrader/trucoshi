...
import { projectPublicAccount } from "../../accounts/publicAccount"
...
import { MatchChatAnnouncements } from "./MatchChatAnnouncements"
import { getBetsEnabled, getMaxBetSats, getRakePercent, validateOpsTokens } from "../config"
import { createRuntimeRequestHandler, RuntimeOpsStatus } from "../RuntimeOps"
import {
  assertBoolean,
...
const createMatchSessionId = () => randomUUID().replace(/-/g, "").slice(0, 12)
...
export interface ITrucoshi {
  ...
  isAcceptingNewGames(): boolean
  assertAcceptingNewGames(): void
  getOpsStatus(): RuntimeOpsStatus
  drainAdmission(): Promise<RuntimeOpsStatus>
  resumeAdmission(): Promise<RuntimeOpsStatus>
  markDraining(): void
  close(): Promise<void>
  listen: (
...
export const Trucoshi = ({
  ...
  origin,
  serverVersion,
  betsEnabled = getBetsEnabled(),
  maxBetSats = getMaxBetSats(),
  rakePercent = getRakePercent(),
  opsToken = process.env.APP_OPS_TOKEN,
  opsStatusToken = process.env.APP_OPS_STATUS_TOKEN,
}: {
  port: number
  ...
  if (maxBetSats !== undefined && (!Number.isSafeInteger(maxBetSats) || maxBetSats < 1)) {
    throw new Error("maxBetSats must be a positive safe integer when configured")
  }
  if (!Number.isFinite(rakePercent) || rakePercent < 0 || rakePercent > 100) {
    throw new Error("rakePercent must be between 0 and 100")
  }
  if (process.env.NODE_ENV === "production" && betsEnabled && maxBetSats === undefined) {
    throw new Error("APP_MAX_BET must be set to a positive limit when production bets are enabled")
  }
  validateOpsTokens(opsToken, opsStatusToken)

  const health = new RuntimeHealth(serverVersion)
  let server: ITrucoshi
  const httpServer = createServer(
    createRuntimeRequestHandler(health, {
      token: opsToken,
      statusToken: opsStatusToken,
      getStatus: () => server.getOpsStatus(),
      drain: () => server.drainAdmission(),
      resume: () => server.resumeAdmission(),
    })
  )
  ...
  let acceptingNewGames = true
  let inFlightAdmissions = 0
  let admissionIdleResolvers: Array<() => void> = []
  let admissionDrainPromise: Promise<void> | undefined
  let closePromise: Promise<void> | undefined

  const assertBetAllowed = (satsPerPlayer: number | undefined) => {
    if (satsPerPlayer === undefined) {
      return
    }
    if (!Number.isSafeInteger(satsPerPlayer) || satsPerPlayer < 0) {
      throw new SocketError("FORBIDDEN", "La apuesta debe ser una cantidad entera valida")
    }
    if (!betsEnabled && satsPerPlayer !== 0) {
      throw new SocketError("FORBIDDEN", "Las partidas con sats estan deshabilitadas")
    }
    if (maxBetSats !== undefined && satsPerPlayer > maxBetSats) {
      throw new SocketError("FORBIDDEN", `Maximo ${maxBetSats} sats`)
    }
  }

  const beginAdmissionOperation = () => {
    server.assertAcceptingNewGames()
    ...
    return () => {
      ...
      if (inFlightAdmissions === 0) {
        const resolvers = admissionIdleResolvers
        admissionIdleResolvers = []
        resolvers.forEach((resolve) => resolve())
      }
    }
  }

  const waitForAdmissionIdle = () =>
    inFlightAdmissions === 0
      ? Promise.resolve()
      : new Promise<void>((resolve) => admissionIdleResolvers.push(resolve))
  ...
  server = {
    ...
    isAcceptingNewGames() {
      return acceptingNewGames
    },
    assertAcceptingNewGames() {
      if (!acceptingNewGames) {
    ...
    getOpsStatus() {
      ...
      for (const table of server.tables.values()) {
        const state = table.state()
        if (state === EMatchState.STARTED || state === EMatchState.PAUSED) {
          activeMatches += 1
          activePlayers += table.lobby.players.length
        } else if (state === EMatchState.UNREADY || state === EMatchState.READY) {
          waitingMatches += 1
          waitingPlayers += table.lobby.players.length
        }
      }

      return {
        version: server.health.snapshot().version,
        admission: acceptingNewGames ? "accepting" : "draining",
        acceptingNewGames,
        betsEnabled: server.betsEnabled,
        counts: {
          activeMatches,
          waitingMatches,
          activePlayers,
          waitingPlayers,
          queuedPlayers: server.matchQueue.size,
          queueProposals: 0,
          inFlightAdmissions,
        },
      }
    },
    async drainAdmission() {
      if (!admissionDrainPromise) {
        admissionDrainPromise = server.matchmakingQueue
          .queue(async () => {
            // Change admission state while holding the queue lock. Any match already being
            // assembled by the queue finishes before draining starts, avoiding partial tables.
            acceptingNewGames = false

            for (const entry of Array.from(server.matchQueue.values())) {
              server.removeQueueEntry(entry.userSession.session)
              entry.socket.emit(EServerEvent.QUEUE_UPDATE, null)
            }

            await waitForAdmissionIdle()
          })
          .finally(() => {
            admissionDrainPromise = undefined
          })
      }

      await admissionDrainPromise
      return server.getOpsStatus()
    },
    ...
    async joinQueue({ socket, userSession, options }) {
      assertJoinQueueOptions(options)
    ...
    close() {
      ...
      closePromise = (async () => {
        server.health.beginDraining()
        // Acquire the matchmaking lock before changing admission state. This lets a
        // queue match already being assembled finish atomically before shutdown.
        await server.drainAdmission()
        ...
        if (server.store) {
          closeOperations.push(server.store.$disconnect())
        }
        if (pubClient.isOpen) {
          closeOperations.push(pubClient.close())
        }
        if (subClient.isOpen) {
          closeOperations.push(subClient.close())
        }
        ...
        if (errors.length) {
          throw new AggregateError(errors, "Failed to close Trucoshi cleanly")
        }
      })().finally(() => {
        server.health.markStopped()
      })
    ...
    async listen(
      ...
      server.health.configureDependencies({
        redis,
        postgres: store,
        lightningAccounts,
      })
      ...
      let lightningAccountsInitialized = !lightningAccounts
      let postgresInitialized = !store

      if (lightningAccounts) {
        logger.debug("Initializing lightning-accounts")
        try {
          const profile = await accountsApi.auth.getUserProfile()
          if (betsEnabled) {
            if (!profile.data.id) {
              throw new Error("Lightning Accounts application profile has no id")
            }
            await accountsApi.users.getUserWallet(String(profile.data.id))
          }
          lightningAccountsInitialized = true
          server.health.markDependencyReady("lightningAccounts")
          logger.info("Logged in to lightning-accounts")
        } catch (e) {
          server.health.markDependencyFailed("lightningAccounts")
          logger.error(e, "Failed to initialize lightning-accounts")
        }
      }
      ...
      if (redis) {
        ...
        try {
          ...
          server.health.markDependencyReady("redis")
          logger.info("Connected to Redis")
        } catch (e) {
          server.health.markDependencyFailed("redis")
      ...
      if (store) {
        ...
        try {
          ...
          server.health.markDependencyReady("postgres")
          logger.info("Connected to Postgres")
        } catch (e) {
          server.health.markDependencyFailed("postgres")
          logger.error(e, "Failed to connect to Postgres")
        }

        if (postgresInitialized && lightningAccounts && lightningAccountsInitialized) {
          try {
                ...
                async (tx) => {
                  ...
                  for (const player of match.players) {
                    if (!player.accountId || !player.satsPaid || !player.payRequestId) {
                      ...
                      if (player.satsPaid > 0) {
                        allRefundsResolved = false
                      } else {
                    ...
                    try {
                      ...
                      if (!pr.data) {
                        ...
                        matchLog.warn(
                          { payRequestId: player.payRequestId },
                          "Pay request was not accessible; preserving unresolved bet"
                        )
                        continue
                      }
                      matchLog.info(
                    ...
                    if (pr.data.paid) {
                      ...
                      matchLog.warn(
                        { payRequestId: player.payRequestId },
                        "Pay request is not paid; preserving unresolved bet"
                      )
                    }
                  }

                  const hasAwards = match.players.some((p) => p.satsReceived && p.satsReceived > 0)
                  if (!hasAwards && allRefundsResolved) {
                    ...
                    matchLog.warn(
                      {
                        matchId: match.id,
                        allRefundsResolved,
                        playerIdsWithAwards: match.players
                          .filter((p) => p.satsReceived > 0)
                          .map((p) => p.id),
                      },
                      "Keeping match bet unresolved"
                    )
                  }
                },
                { timeout: 60000 }
        ...
        } else if (postgresInitialized) {
          logger.warn("Skipping unresolved bet reconciliation: lightning-accounts is unavailable")
        }
      }

      if (lightningAccountsInitialized && postgresInitialized && lightningAccounts && store) {
        try {
          await server.getRanking()
        } catch (e) {
          logger.error(e, "Failed to load ranking during startup")
        }
      }

      server.chat = Chat(io, tables)
      callback(io)
      server.health.completeInitialization()
      return io
    },
      ...
      return server.tables
        ...
        .map((match) => {
          ...
          if (socket) {
            ...
            socket.data.matches.add(match.matchSessionId)
          }
          return info
        })
    },
    ...
    logout(socket) {
      ...
      log.debug(
        ...
        "Socket has logged out of account"
      )
    },
      ...
      return new Promise<void>((resolve, reject) => {
        return server
          .getTableSockets(table, async (playerSocket, player) => {
            ...
            if (!playerSocket.data.matches.has(table.matchSessionId)) {
              return
            }
            ...
            playerSocket.emit(
              ...
              (data) => {
                ...
                server
                  ...
                  .catch((e) => {
                    table.log.warn(
                      { ...safeErrorDetails(e), command, playerIdx: player.idx },
          ...
          .catch((e) => table.log.error(safeErrorDetails(e), "emitWaitingPossibleSay error"))
      })
    },
    async emitWaitingForPlay({ play, table, onlyThisSocket }) {
      return new Promise<"say" | "play">((resolve, reject) => {
        ...
        return server
            ...
            if (player.key === play.player?.key) {
              ...
              playerSocket.emit(
                ...
                (data: IWaitingPlayData) => {
                  ...
                  if (!isWaitingPlayData(data)) {
                    table.log.warn(
                      { playerIdx: player.idx, payloadType: typeof data },
                      "Rejected invalid card play payload"
                    )
                    return reject(new Error("Invalid card play payload"))
                  }
                  ...
                  server
                    ...
                    .catch((e) => {
                      table.log.warn(
                        { ...safeErrorDetails(e), cardIdx, playerIdx: player.idx },
                        "Failed to play card"
                      )
                      reject(e)
                    })
          ...
          .catch((e) => table.log.error(safeErrorDetails(e), "emitWaitingForPlay error"))
      })
    },
    ...
    async resetSocketsMatchState(table) {
      await server.getTableSockets(table, async (playerSocket, player) => {
        ...
        playerSocket.data.matches.add(table.matchSessionId)
      })
    },
    ...
    async onBotTurn(table, play) {
      return new Promise(async (resolve, reject) => {
        ...
        const guardedPlayCard: ITrucoshi["playCard"] = (input) => {
          if (settled || !play.waitingPlay) {
            table.log.trace(
              {
                matchSessionId: table.matchSessionId,
                player: input.player.getPublicPlayer("log"),
                card: input.card,
                cardIdx: input.cardIdx,
                waitingPlay: play.waitingPlay,
                settled,
              },
              "Ignoring stale bot card action"
            )
            return Promise.resolve()
          }

          return server.playCard(input)
        }

        const guardedSayCommand: ITrucoshi["sayCommand"] = (input, force) => {
          if (settled || !play.waitingPlay) {
            table.log.trace(
              {
                matchSessionId: table.matchSessionId,
              ...
              "Ignoring stale bot command action"
            )
            return Promise.resolve()
          }

          return server.sayCommand(input, force)
        }
            ...
            .catch((e) => {
              if (settled) {
                table.log.trace(
                  ...
                  "Ignored bot action error after turn settled"
                )
                return resolveOnce()
              }
              rejectOnce(e)
    ...
    clearTurnTimeout(matchSessionId) {
      const turn = server.turns.get(matchSessionId)
      if (turn?.timeout) {
    ...
    setTurnTimeout({ table, play, user, resolve, turn, cancel }) {
      ...
      const isPaused = table.lobby.paused

      if (!isPaused) {
        turn()
      }

      let trucoshiTurn: TrucoshiTurn

      const isActiveTurn = () => server.turns.get(table.matchSessionId) === trucoshiTurn
      ...
      function createTimeout(pausedTime: number) {
        ...
        return setTimeout(() => {
          if (!isActiveTurn()) {
            table.log.trace(
              { matchSessionId: table.matchSessionId, player: play.player?.getPublicPlayer("log") },
              "Ignoring stale turn timeout callback"
            )
            return
          }
          ...
          user
            ...
            .then(() => {
              if (!isActiveTurn()) {
                table.log.trace(
                  { matchSessionId: table.matchSessionId, player: tPlayer.getPublicPlayer("log") },
                  "Ignoring stale turn reconnection success callback"
                )
                return
              }
              ...
              if (!currentTurn) {
                table.log.trace(
                  { matchSessionId: table.matchSessionId, player: tPlayer.getPublicPlayer("log") },
                  "Skipping reconnection continuation because turn no longer exists"
                )
                return
              }
            ...
            .catch(() => {
              if (!isActiveTurn()) {
                table.log.trace(
                  { matchSessionId: table.matchSessionId, player: tPlayer.getPublicPlayer("log") },
                  "Ignoring stale turn reconnection timeout callback"
                )
                return
              }
          ...
          table.log.error(
            {
              ...
              hasPlay: !!play,
              hasPlayer: !!play?.player,
            },
            "Skipping onTurn: missing session, play instance or player"
          )
          return resolve()
        }
      ...
      return new Promise<void>((resolve) => {
        ...
        if (!session || !play || !play.player) {
          table.log.error(
            ...
            "Skipping onTruco: missing session, play instance or player"
          )
          return resolve()
        }
      ...
      return new Promise<void>((resolve) => {
        ...
        if (!session || !play || !play.player) {
          table.log.error(
            ...
            "Skipping onFlor: missing session, play instance or player"
          )
          return resolve()
        }
      ...
      return new Promise<void>((resolve) => {
        ...
        if (!session || !play || !play.player) {
          table.log.error(
            ...
            "Skipping onEnvido: missing session, play instance or player"
          )
          return resolve()
        }
    ...
    async createMatchTable(userSession, socket, options = {}) {
      assertMatchCreationOptions(options)
      const endAdmissionOperation = beginAdmissionOperation()
      try {
        assertBetAllowed(options.satsPerPlayer)
        let matchSessionId = createMatchSessionId()
        while (server.tables.get(matchSessionId)) {
          matchSessionId = createMatchSessionId()
        }
        ...
        endAdmissionOperation()
      }
    },
    async addBot(table, userSession, teamIdx) {
      teamIdx = normalizeOptionalTeamIndex(teamIdx)
      const endAdmissionOperation = beginAdmissionOperation()
      try {
    ...
    async setMatchOptions({ socket, matchSessionId, userSession, options, emitChat = true }) {
      ...
      assertBetAllowed(satsPerPlayer)
            ...
            if (dbMatch.bet && dbMatch.bet.satsPerPlayer > 0 && hasChangedBet) {
              for (const player of dbMatch.players) {
                if (player.payRequestId && player.satsPaid > 0) {
                  ...
                  if (!pr.data) {
                    throw new SocketError("UNEXPECTED_ERROR", "Pay request not found")
                  }
    ...
    async pauseMatch({ matchSessionId, userSession, pause }) {
      ...
      if (pause) {
        ...
        const promise = table.lobby
          ...
          .then(({ emitter, playerNotAlone }) => {
            ...
            server
              .emitMatchUpdate(table)
              .catch((e) =>
                table.log.error(safeErrorDetails(e), "Failed to emit UPDATE_MATCH after pause")
              )
          ...
          .catch((e) => {
            ...
            table.log.error(safeErrorDetails(e), "Failed to pause match")
            return false
          })
        ...
        const pauseRequestTimeout = setTimeout(() => {
          if (table.lobby.pauseRequest) {
            clearTimeout(table.lobby.pauseRequest.pauseTimeout)
        ...
        setTimeout(() => {
          server
            .getTableSockets(table, async (socket, socketPlayer) => {
              if (socketPlayer && table.lobby.pauseRequest && !table.lobby.paused) {
                socket.emit(
            ...
            .catch((e) =>
              log.fatal(
                { ...safeErrorDetails(e), matchSessionId },
      ...
      return table.lobby
        ...
        .then(({ emitter }) => {
          ...
          chat.system("La partida se ha reanudado...", "menu1")

          turn.retry(pausedTime)
          server
            .emitMatchUpdate(table)
            .catch((e) =>
              table.log.error(safeErrorDetails(e), "Failed to emit UPDATE_MATCH after unpause")
            )
          return false
        })
        .catch((e) => {
          table.log.error(
            { ...safeErrorDetails(e), matchSessionId },
            "Failed to unpause match"
          )
          return false
        })
        .finally(() => {
          if (table.lobby.pauseRequest?.pauseTimeout) {
            clearTimeout(table.lobby.pauseRequest.pauseTimeout)
          }
          table.lobby.pauseRequest = undefined
        })
    ...
    async kickPlayer({ matchSessionId, userSession, key }) {
      assertStringIdentifier(matchSessionId, "matchSessionId")
      assertStringIdentifier(key, "player key")
      const table = server.tables.getOrThrow(matchSessionId)
      const player = table.isSessionPlaying(userSession.session)
      ...
      server
        .emitMatchUpdate(table)
        .catch((e) =>
          table.log.error(safeErrorDetails(e), "Emit update after kick player failed")
        )
    ...
    async setMatchPlayerReady({ matchSessionId, userSession, ready }) {
      ...
      if (server.store) {
          ...
          if (dbPlayerExists && dbPlayerExists.satsPaid > 0 && dbPlayerExists.payRequestId) {
            ...
            try {
              ...
              table.log.debug(
                ...
                "Found pay request for setting player ready"
              )
              ...
              table.log.error(
                { ...safeErrorDetails(e), payRequestId: player.payRequestId },
        ...
        table.log.trace(
          {
            matchPlayer: {
              ...
              teamIdx: update.teamIdx,
              bot: update.bot,
              satsPaid: update.satsPaid,
              payRequestId: update.payRequestId,
            },
          },
          "About to update or create match player"
        )
        ...
        try {
          if (dbPlayerExists) {
            ...
            table.log.trace(
              { playerId: dbPlayer.id, accountId: dbPlayer.accountId },
              "Updated match player"
            )
          } else {
            ...
            table.log.trace(
              { playerId: dbPlayer.id, accountId: dbPlayer.accountId },
              "Created match player"
            )
          }
        } catch (e: any) {
          table.log.error(safeErrorDetails(e), "Failed to update or create match player")
      ...
      try {
        ...
        table.log.error(
          safeErrorDetails(e),
          "Failed to set player ready or emit match update"
        )
    ...
    async joinMatch(table, userSession, socket, teamIdx) {
      ...
      if (!server.isAcceptingNewGames()) {
        ...
        socket.join(table.matchSessionId)
        socket.join(table.matchSessionId + currentPlayer.teamIdx)
        socket.leave(table.matchSessionId + getOpponentTeam(currentPlayer.teamIdx))
        socket.data.matches?.add(table.matchSessionId)
        userSession.reconnect(table.matchSessionId, "disconnection")
        table.playerReconnected(currentPlayer, userSession)
        await server.emitMatchUpdate(table)
      ...
      const endAdmissionOperation = beginAdmissionOperation()
      try {
        assertBetAllowed(table.lobby.options.satsPerPlayer)
        ...
        server
          .emitMatchUpdate(table)
          .catch((e) => table.log.error(e, "Failed to emit match update after player joined"))
    ...
    async startMatch({ identityJwt, matchSessionId, userSession }) {
      ...
      try {
        ...
        if (hasBet) {
          ...
          const expectedPayRequestIds = new Set(payRequestIds)
          const returnedPayRequestIds = new Set(prs.data.map((payRequest) => payRequest.id))
          const responseHasEveryRequestedPayRequest =
            expectedPayRequestIds.size === payRequestIds.length &&
            prs.data.length === payRequestIds.length &&
            returnedPayRequestIds.size === expectedPayRequestIds.size &&
            prs.data.every(
              (payRequest) =>
                payRequest.id !== undefined && expectedPayRequestIds.has(payRequest.id)
            )

          if (!responseHasEveryRequestedPayRequest || prs.data.some((pr) => !pr.paid)) {
        ...
        const matchChatAnnouncements = MatchChatAnnouncements()

        table.lobby
          .startMatch()
          .onBeforeStarts(async () => {
            await server.emitMatchUpdate(table)
          ...
          .onTurn(async (play) => {
            ...
            const messages = matchChatAnnouncements.getHandStartMessages(
              play,
              table.lobby.players
            )
            messages.forEach((message) => chat?.system(message))
          ...
          .onWinner(async (winnerTeam, points) => {
            if (server.store) {
              ...
              if (satsPerPlayer > 0) {
                ...
                const tax = Math.round((pool * rakePercent) / 100)
                const prize = pool - tax
                const winnersLength = winnerTeam.players.filter((p) => !p.abandoned).length
                if (winnersLength < 1) {
                  throw new Error("Cannot settle a match without an eligible winner")
                }
                const amountInSats = Math.floor(prize / winnersLength)

                  table.log.debug(
                  { pool, tax, prize, amountInSats, winnersLength, rakePercent },
                    "Paying winner award"
                  )
              ...
              server
                .getRanking()
                .catch((e: any) =>
                  logger.error(safeErrorDetails(e), "Failed to get ranking after match ended")
                )
            }
        ...
        server
          ...
          .catch((e) => table.log.error(e, "Failed to inspect sockets after match creation"))
    ...
    async playAgain({ matchSessionId, userSession, socket }) {
      ...
      const isPlayer = table.lobby.players.find((p) => p.session === userSession.session)
      ...
      try {
        if (table.lobby.playAgainRequest) {
          const newMatch = server.tables.getOrThrow(table.lobby.playAgainRequest.newMatchSessionId)
          const possible = _getPossiblePlayingMatch(newMatch.matchSessionId, userSession.session)

          if (!possible?.player) {
            await server.joinMatch(newMatch, userSession, socket)
          }

          if (!table.lobby.playAgainRequest.acceptedBySessions.has(userSession.session)) {
    ...
    async deletePlayerAndReturnBet(table, player) {
      try {
        ...
        if (!player.accountId) {
          table.log.trace(
            { playerId: player.id, matchSessionId: table.matchSessionId },
        ...
        if (!pr.data) {
          throw new SocketError("PAYMENT_ERROR", "Pay request not found")
        }
        if (!pr.data.paid) {
          table.log.error(
    ...
    async leaveMatch(matchId, socket, force = false) {
      assertStringIdentifier(matchId, "matchSessionId")
      const table = server.tables.get(matchId)
        ...
        if (player) {
          if (notPlaying) {
            ...
            userSession
              ...
              .finally(() =>
                server
                  .emitMatchUpdate(table)
                  .catch((e) => log.error(e, "Failed to emit match update after cleanup"))
              )
            return
          }
          ...
          if (force) {
            ...
            server
              .emitMatchUpdate(table)
              .catch((e) =>
                log.error(safeErrorDetails(e), "Failed to emit match update after a player left")
              )
          }
        }
      } catch (e) {
    ...
    async getMatchDetails(socket, matchId) {
      assertPositiveIntegerIdentifier(matchId, "matchId")
      if (!server.store) {
    ...
    async getAccountDetails(socket, accountId) {
      assertPositiveIntegerIdentifier(accountId, "accountId")
      if (!server.store) {
        throw new SocketError("NOT_FOUND", "Este server no soporta historial")
      }
      ...
      return {
        stats,
        matches,
        account: projectPublicAccount(account.data),
      }
    },
    async getRanking() {
      if (!server.store) {
