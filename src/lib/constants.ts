...
export const ENVIDO = {
  ...
  [ESayCommand.ENVIDO_ENVIDO]: () => ({
    accept: 4,
    decline: 2,
  }),
  [ESayCommand.REAL_ENVIDO]: () => ({
    accept: 3,
    decline: 1,
  }),
  [ESayCommand.FALTA_ENVIDO]: (points: any, matchPoint: number) => {

    const max = Math.max(...points as Array<number>)

    return {
      accept: matchPoint - max,
      decline: 2,
    }
  },
}
